...
** DONE Make sure the symbols tool handles underscore in names. (we use \w* in the patterns, which does not include underscore)
CLOSED: [2016-11-17 Thu 17:44]
Wrong, \w handles underscore: http://www.cplusplus.com/reference/regex/ECMAScript/
** DONE Re-organize configuration files of the demo — factor out pancake.h and astar.h. Then create grid.astar.h etc.
CLOSED: [2016-11-21 Mon 12:34]
** DONE Why does idastar.copy require /neighbors/ instead of /actions/. Doesn’t the backtracking policy do the job?
CLOSED: [2016-11-21 Mon 13:02]
** DONE Make it unnecessary to define the domain symbols, i.e. to conditionally include domains. 
CLOSED: [2016-11-23 Wed 12:01]
** DONE Fix a big design error: it is possible for the solution path to not be available or to be computed incorrectly, including the possibility of an infinite loop in event_base.h:134. This is because an event involving the parent of a state may appear after the last event involving the state. So, with each event, we need to store the last event that involved the state’s parent.
CLOSED: [2016-11-24 Thu 12:22]
** DONE Move grid.h to extensions and move Octile heuristic to it. Also move computation of coordinates for visualization there. 
CLOSED: [2016-11-26 Sat 19:38]
** DONE Make names of include guards uniform. 
CLOSED: [2016-11-27 Sun 11:51]
** DONE Make the size of vertices commensurate with the number of them.
CLOSED: [2016-11-27 Sun 12:11]
For now, just lowered the percentile to 0.1%. Making vertices smaller would make the grid map look too bad.
** DONE Change the names in configs related to neighbor generation to be less cryptic.
CLOSED: [2016-11-27 Sun 12:26]
** DONE Make final check that all (really all) demo versions run with and without -v 0.
CLOSED: [2016-11-27 Sun 12:26]
- Pancake with 30 pancakes and visualization seg-faults very quickly. Fixed.
** DONE Update the video.
CLOSED: [2016-11-28 Mon 12:59]
** DONE Fix warnings in doxygen output. Note how TCLAP began to produce warnings.
CLOSED: [2016-11-30 Wed 17:48]
** DONE Proofread the paper.
CLOSED: [2016-12-01 Thu 17:53]
** DONE Make changes in the documentation: 
CLOSED: [2016-12-05 Mon 12:46]
- A note about the namespaces matching the physical structure.
  + Done.
- A note about the need to make functions that read specific command line arguments templates. As an example of that, constructors of the domains that read domain-specific command line arguments are templates. The functions that invoke those are templates as well.
  + Done.
- The symbols tool automatically produces the types for the default template arguments.
  + Decided to not provide this detail.
** DONE Proofread the main page of the online documentation.
CLOSED: [2016-12-05 Mon 12:47]
** TODO Put software and documentation on github.
** TODO Update the paper’s metadata.
** TODO Submit
* After submission
