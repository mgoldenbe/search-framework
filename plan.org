...
** TODO Fix a big design error: it is possible for the solution path to not be available or to be computed incorrectly, including the possibility of an infinite loop in event_base.h:134. This is because an event involving the parent of a state may appear after the last event involving the state. So, with each event, we need to store the last event that involved the state’s parent.
