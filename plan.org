...
* Remains on 27.09:
Each thing on this list is not long. They should take one day on average. The days remaining until Sukkot are: 28.09–30.09, 5.10–7.10, 9.10–10.10, 13–14.10. So, 10 days, which are also very busy. There are 11 items on the list, so I should expect to submit the paper in the first two weeks after Sukkot, e.g. by 4.11. I will then update k-goal config files to work with Roni. 
** Organize folders. 
Make sure all demo versions compile and run with and without -v 0. Make a python script for this check.
** Introduce necessary namespaces and make the naming consistent (singular/plural).
There should be a namespace for each policy.
** Make a note in documentation about the possible deviation of configs in video from the current configs.
** Make sure that std::move is used wherever needed (i.e. wherever right reference is used). 
** Document the components of IDA*.
** Document utils/.
** Make sure that everything is documented.
** Either add to bugs or fix the issue with layout computation not converging.
** Improve the symbols tool.
** Proofread the paper.
** Submit
* After submission
