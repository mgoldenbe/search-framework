#ifndef SLB_EXTENSIONS_EVENTS_EVENT_TYPES_H
#define SLB_EXTENSIONS_EVENTS_EVENT_TYPES_H
...
template <class Node = SLB_NODE> struct NoChange : EventBase<Node> {
    using EventBase<Node>::EventBase; ///< Using the base's constructors.
};

/// An event that applies the same visual changes to all participating states and
/// edges.
/// \tparam Node The search node type.
template <class Node = SLB_NODE> struct UniformChange : EventBase<Node> {
    /// The state type, represents the domain.
    using State = typename Node::State;

    /// Shared pointer to constant state.
    using StateSharedPtr = core::util::deref_shared_ptr<const State>;
    ...
    using EventBase<Node>::EventBase;

    /// The type for holding all the visual changes associated with the event.
    using VisualChanges = typename EventBase<Node>::VisualChanges;
...
template <class Node = SLB_NODE> struct VertexEdgeChange : UniformChange<Node> {
    ...
    using EventBase<Node>::parent_;

    /// Using the \c parentSubstitution_ variable of the base.
    using EventBase<Node>::parentSubstitution_;
...
protected:
    ...
    virtual StateSet path() const {
        return EventBase<Node>::path(state_);
    };
};
