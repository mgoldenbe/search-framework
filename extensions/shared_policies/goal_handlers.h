...
struct MinHeuristicGoalHandler {
    ...
    /// Type of an active goal. An active goal consists of the goal state and
    /// the goal's index.
    ...
    MinHeuristicGoalHandler(MyAlgorithm &alg) : alg_(alg), log_(alg.log()) {
...
private:
    ...
    /// Flag showing whether the search algorithm has accomplished its
    /// mission
    /// and should terminate.
    bool done_ = false;
    ...
    void doneGoal(Node *n) {
        ...
        res = (res * (nDoneGoals - 1) + n->f) / nDoneGoals;
