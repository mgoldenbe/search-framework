...
The heuristic policies provide the call operator for computing the heuiristic. See the file \hyperlink{heuristic__policies_8h}{heuristic\+\_\+policies.\+h} for some examples of heuristic policies.
\item {\bfseries The generator} ({\ttfamily Generator\+\_\+}) specifies the process by which the successors (also called {\itshape neighbors}) of a state are gererated. The list of successors may consist either of the neighboring states or of the actions that lead to those states. Regardless of this distinction, the algorithm or other policies (see \hyperlink{index_s-communication}{4.\+5.\+2 Communication through reference to algorithm}) may explicitly call the following services of the generator\+:
\begin{DoxyItemize}
...
\begin{DoxyItemize}
\item The \hyperlink{structslb_1_1ext_1_1algorithm_1_1Astar}{A$\ast$} algorithm uses the open list policy called {\bfseries Open\+\_\+}, which determines the particular open list structure used by this algorithm. The open list policy provides the usual open list operations. See the file \hyperlink{open__list_8h}{open\+\_\+list.\+h} for an example of an open list policy.
\item The \hyperlink{structslb_1_1ext_1_1algorithm_1_1IdAstar}{I\+D\+A$\ast$} algorithm uses the pruning policy called {\bfseries Pruning}, which specifies how the algorithm is to determine whether a successor can be pruned. Note that we are not talking about the pruning based on the cost threshold of the current iteration, which is done by the algorithm itself. See the file \hyperlink{pruning_8h}{pruning.\+h} for some examples of pruning policies.
\end{DoxyItemize}\hypertarget{index_s-events}{}\subsubsection{4.\+6 Events}\label{index_s-events}
